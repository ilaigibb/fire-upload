# Install Fire Upload on Proxmox
...
2. Create a [fine-grained GitHub token](https://github.com/settings/personal-access-tokens/new):
   - Repository access: **Only select repositories → `fire-upload`**
...
(
  set -e
  installer="$(mktemp /root/install-fire-upload.XXXXXX.sh)"
  trap 'rm -f -- "$installer"' EXIT

  read -rsp "Paste GitHub token: " FIRE_UPLOAD_GITHUB_TOKEN
  echo
  ...
  chmod 700 "$installer"
  FIRE_UPLOAD_GITHUB_TOKEN="$FIRE_UPLOAD_GITHUB_TOKEN" bash "$installer"
)
```
...
Do not forward Proxmox port 8006 or Fire Upload port 8080.
